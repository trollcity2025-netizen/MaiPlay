...
export function LiveViewerPage() {
  ...
  const sessionQuery = useQuery({
    ...
    queryFn: async (): Promise<LiveSession | null> => {
      ...
      if (error) {
        console.error('[LiveViewerPage] Failed to load live session:', error)
        return null
      }
  ...
  const isHost = !!user?.id && !!creatorId && user.id === creatorId
  ...
  const broadOfficerQuery = useQuery({
    ...
    queryFn: async (): Promise<boolean> => {
      ...
      if (error) {
        console.error('[LiveViewerPage] Failed to check BroadOfficer status:', error)
        return false
      }
  ...
  const relatedVideosQuery = useQuery({
    ...
    queryFn: async (): Promise<RelatedVideo[]> => {
      ...
      if (error) {
        console.error('[LiveViewerPage] Failed to load related videos:', error)
        return []
      }
   ...
   const handleHostJoined = useCallback(() => {
     console.info('[LiveViewerPage] Host joined live session:', liveSessionId)
   }, [liveSessionId])

   const handleHostLeft = useCallback(() => {
     console.info('[LiveViewerPage] Host left live session:', liveSessionId)
   }, [liveSessionId])
